...
export default function AccountDashboard() {
  ...
  async function handleCreateGroup() {
    ...
    function openSavedSplit(save: CloudSavedSplit) {
        ...
        const sp = new URLSearchParams({ s: param });

        router.push(`/split/new?${sp.toString()}`);
    }
...
function GroupAvatar({
  ...
  return (
    <div
      className={[
        "grid shrink-0 place-items-center overflow-hidden bg-sky-100 text-indigo-800 shadow-sm ring-1 ring-black/5",
        compact
          ? "h-14 w-14 rounded-[1.15rem] text-xl"
          : "h-14 w-14 rounded-[1.25rem] text-2xl sm:h-16 sm:w-16 sm:rounded-[1.35rem] sm:text-3xl",
      ].join(" ")}
    >
      {group.photo_url ? (
  ...
  return (
    <div className="mb-3 flex items-center justify-between gap-3 sm:mb-5">
      <div className="flex min-w-0 items-center gap-2.5">
        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-2xl bg-teal-600 text-sm font-bold text-white shadow-sm">
          K
        </div>
        ...
        <div className="min-w-0">
          ...
          <div className="truncate text-[10px] text-zinc-500 sm:text-xs">
            Your saved groups, folders, and splits
          </div>
        </div>
      </div>

      <div className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-zinc-600 shadow-sm">
        {label}
      </div>
    </div>
  );
